...
> ⚠️ ЗАМЕНЁН планом `done/unlock_am_data/` (реализован 2026-06-10).
> Фазы C/D этого плана УСТАРЕЛИ (использовали fuzzy при импорте + межисточниковую
> транслитерацию). Итоговое решение — изоляция через `runners.source` + нормализация
> в родном алфавите. Оставлен для истории. НЕ выполнять.

> Агент: Data Engineer + Backend
> Зависит от: нет
> Статус: superseded (см. unlock_am_data)
