...
**Что ищем:**
- [ ] Точные дубли `name_normalized` — не должно быть (это баг в batch_parse)
- [ ] Fuzzy дубли (Knyaz/Kniaz) — список для ручного мёрджа
- [ ] Кириллические имена — не должно быть в CLAX данных (если есть — откуда?)
- [ ] Односложные имена — подозрительные
- [ ] Формат: "LastName FirstName" или "FirstName LastName"?
...
**Что ищем:**
- [ ] Клубы-города — должны быть удалены (это из AM данных)
- [ ] Кириллические клубы — откуда? CLAX отдаёт на латинице
- [ ] Дубли клубов ("SRG" vs "S.R.G.", "SkyRunGroup" vs "SKYRUN TEAM")
- [ ] Клубы с 1 бегуном — шум, но может быть ок
...
**Решение:** Если AM данные ещё есть — удалить (SQL из MEMORY.md).
...
- [ ] 0 точных дублей `name_normalized`
- [ ] 0 клубов-городов (Алматы, Астана и т.д.)
- [ ] 0 кириллических имён в runners (если есть — разобраться откуда)
- [ ] Fuzzy дубли — список составлен, очевидные смёрджены
- [ ] AM данные удалены (если были)
- [ ] Таблицы `runner_name_aliases` и `club_name_aliases` созданы
- [ ] Алиасы заполнены из текущих CLAX данных
- [ ] `batch_parse.py` пишет алиасы при сохранении
- [ ] Поиск работает через алиасы (проверить на портале)
- [ ] Бегуны с несколькими вариантами написания — находятся по любому
...
1. [ ] После парсинга имени → `transliterate_cyrillic(name)` для `name` в результате
2. [ ] Сохранять **оригинальное кириллическое имя** — оно попадёт в алиас автоматически
   (batch_parse сохранит и транслитерированный вариант как name, и кириллический как алиас)
3. [ ] Парсить birth_year из возрастной категории AM (если доступна) — для валидации мёрджей
...
У АМ нет поля "клуб" (club=None в парсере).
Клуб у бегуна = клуб из CLAX гонки (если бежал).
City из АМ НЕ маппится в club.
